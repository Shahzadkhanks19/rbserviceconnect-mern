...
function completion(profile,user){
  const checks=[user.firstName,user.lastName,profile?.headline,profile?.phone,profile?.location,profile?.bio,profile?.skills?.length,profile?.experience?.length,profile?.education?.length,profile?.resume?.url,profile?.preferences?.jobTypes?.length,profile?.preferences?.workplaces?.length];
...
export async function getOverview(req,res){
  const [profile,applications]=await Promise.all([getOrCreateProfile(req.user._id),Application.find({candidate:req.user._id}).sort({updatedAt:-1}).limit(8).lean()]);
  const activeApplications=applications.filter((item)=>!['rejected','withdrawn','hired'].includes(item.status)).length;
  ...
  return res.json({profileCompletion:completion(profile,req.user),counts:{activeApplications,savedJobs:savedJobs.length,interviews,totalApplications:applications.length},recentApplications:applications.slice(0,4),savedJobs:savedJobs.slice(0,4),profile});
}
