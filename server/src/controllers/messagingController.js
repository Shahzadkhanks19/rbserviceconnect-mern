...
export async function listConversations(req,res){
  const showArchived=req.query.archived==='true';const filter={...accessFilter(req.user),archivedBy:showArchived?req.user._id:{$ne:req.user._id}};
  const conversations=await Conversation.find(filter).sort({lastMessageAt:-1}).populate('recruiter','firstName lastName email').populate('candidate','firstName lastName email').populate('job','title slug').lean();
...
export async function listMessages(req,res){const conversation=await getAccessibleConversation(req.user,req.params.id);if(!conversation)return res.status(404).json({message:'Conversation not found.'});const readAt=new Date();const result=await Message.updateMany({conversation:conversation._id,sender:{$ne:req.user._id},readAt:null},{$set:{readAt}});const messages=await Message.find({conversation:conversation._id}).sort({createdAt:1}).populate('sender','firstName lastName role').lean();if(result.modifiedCount)req.app.get('io')?.to(`conversation:${conversation._id}`).emit('messages:read',{conversationId:String(conversation._id),readerId:String(req.user._id),readAt});return res.json({conversation,messages});}
