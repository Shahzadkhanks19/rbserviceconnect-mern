...
export async function register(req,res){
  ...
  const approvalRequired=platform?.requireRecruiterApproval!==false;
  const status=role==='recruiter'&&approvalRequired?'pending':'active';
  const user=await User.create({firstName,lastName,email,password,role,status});
  return res.status(201).json({message:role==='recruiter'&&approvalRequired?'Recruiter account created and awaiting approval':'Account created successfully',user:{id:user._id,firstName:user.firstName,lastName:user.lastName,email:user.email,role:user.role,status:user.status}});
}
