...
export function validateEnvironment(){
  ...
  const razorpayKey=String(process.env.RAZORPAY_KEY_ID||'').trim();
  ...
  if(process.env.NODE_ENV==='production'){
    ...
    const cloudinaryMissing=['CLOUDINARY_CLOUD_NAME','CLOUDINARY_API_KEY','CLOUDINARY_API_SECRET'].filter((key)=>!String(process.env[key]||'').trim());
    if(cloudinaryMissing.length)throw new Error(`Cloudinary is required in production for resume and message attachment uploads. Missing: ${cloudinaryMissing.join(', ')}`);
