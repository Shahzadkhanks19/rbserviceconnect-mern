...
const promotionSchema=new mongoose.Schema({
  ...
  payment:{provider:{type:String,trim:true,maxlength:40,default:''},orderId:{type:String,trim:true,maxlength:180,default:''},paymentId:{type:String,trim:true,maxlength:180,default:''},paidAt:{type:Date,default:null}},
},{timestamps:true});
promotionSchema.index({owner:1,status:1,createdAt:-1});
promotionSchema.index({target:1,status:1,endsAt:-1});
promotionSchema.index({product:1,status:1,endsAt:-1});
