...
export default function RegisterPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    setError('');
    if (!form.firstName.trim() || !form.lastName.trim() || !form.email.trim() || !form.password) { setError('Complete all required fields.'); return; }
    if (!passwordChecks.length) { setError('Your password must be at least 8 characters.'); return; }
    ...
    try {
      ...
      await apiRequest('/auth/register', {
        method: 'POST',
        body: JSON.stringify({ firstName: form.firstName.trim(), lastName: form.lastName.trim(), email: form.email.trim(), password: form.password, role }),
      });
    ...
    <section className="bg-slate-50 py-10 sm:py-14">
      <div className="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[.82fr_1.18fr] lg:px-8">
        ...
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl shadow-black/5 sm:p-8">
          ...
          {role === 'recruiter' && <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-800">Employer accounts must verify their email and are then reviewed before recruiter tools are activated.</div>}
