...
export default function JobsPage() {
  ...
  const filteredJobs = useMemo(() => {
    ...
    const result = jobs.filter((job) => {
      const matchesQuery = !normalizedQuery || [job.title, job.company, job.category, ...job.skills]
        .join(' ')
        .toLowerCase()
  ...
  const filterPanel = (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <p className="text-sm font-semibold text-slate-950">Refine results</p>
          <p className="mt-1 text-xs text-slate-500">Narrow jobs by what matters to you.</p>
        </div>
        ...
        <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8 lg:py-16">
          ...
          <div className="mt-8 grid gap-3 rounded-3xl border border-black/5 bg-white p-3 shadow-xl shadow-black/5 lg:grid-cols-[1fr_.8fr_auto]">
            <label className="flex min-w-0 items-center gap-3 rounded-2xl bg-slate-50 px-4 py-3.5 focus-within:ring-2 focus-within:ring-indigo-200">
              ...
              <input value={query} onChange={(event) => setQuery(event.target.value)} className="min-w-0 flex-1 bg-transparent text-sm text-slate-950 outline-none placeholder:text-slate-400" placeholder="Job title, skill, or company" aria-label="Search jobs" />
            </label>
            <label className="flex min-w-0 items-center gap-3 rounded-2xl bg-slate-50 px-4 py-3.5 focus-within:ring-2 focus-within:ring-indigo-200">
              <MapPin size={19} className="shrink-0 text-slate-400" />
              <input value={location} onChange={(event) => setLocation(event.target.value)} className="min-w-0 flex-1 bg-transparent text-sm text-slate-950 outline-none placeholder:text-slate-400" placeholder="City, state, remote" aria-label="Search by location" />
            </label>
            <button type="button" className="rounded-2xl bg-indigo-500 px-6 py-3.5 text-sm font-semibold text-white transition hover:bg-indigo-400">Search jobs</button>
          </div>
        </div>
      </section>
